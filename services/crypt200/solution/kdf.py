...
u = 'probe@mantle.earth'
p = 'ThisPasswordIsJustRandomGibberish'
plain = 'tesseract2014{AlwaysRollYourOwnCrypto}'
encrypted = encrypt('probe@mantle.earth', "ThisPasswordIsJustRandomGibberish", plain)
cipher = encrypted[1]
decrypted = decrypt(u, p, cipher)
assert plain == decrypted and cipher != plain
print cipher
print decrypted

def solve(u, cipher):
    cipher = base64.b64decode(cipher)
    cs1 = chr(sum([ord(i) for i in u]) % 256)
    cs3 = chr(reduce(lambda x,y: x*y, [ord(i) for i in u], 1) % 256)
    for p1 in range(256):
        print p1
        for p2 in range(256):
            cs2 = chr(p1)
            cs4 = chr(p2)
            salt = cs1 + cs2 + cs3 + cs4
            c = pbkdf2("kLen is too long", salt)
            otp = [ord(i) for i in c]    

            msg = []
            for i in range(len(cipher)):
                msg.append(chr(ord(cipher[i]) ^ otp[i % len(otp)]))
            if 'tesseract2014' in ''.join(msg):
                return ''.join(msg)

#print solve(u, cipher)
